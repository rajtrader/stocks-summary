...
function extractStockNameFromCSV(filePath = './dailygainer.csv') {
...
From the above content, Find 3 reasons explaining the daily gainer behaviour, and give each reason in 100 characters with a title of 2 words for each reason.
Format:
...
async function sendToWordPress(stock, stockName,changePercent, reasons, tag = 'dailygainer') {
  try {
...
export async function rundailygainer() {
  const companies = extractStockNameFromCSV('./dailygainer.csv');
