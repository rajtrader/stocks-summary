...
async function getCompanyFeed(stockName) {
  ...
  try {
    ...
    const [rows] = await connection.query(
      `SELECT content FROM wp_stockedge_monthly_loser_investigator
       WHERE stockName = ?
...
async function sendToWordPress(stock, stockName, reasons, tag = 'monthlyloser') {
  try {
...
export async function runmonthlygainer() {
